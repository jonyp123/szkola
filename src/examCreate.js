...
const ExamCreate = () => {
  ...
  const createQuiz = async() => {
    let quizCode =  Math.random() * 1000
    quizCode = quizCode.toString()
    if(questions.length > 0){
      const quizesCollectionRef = collection(db, "quiz")
          const data = await addDoc(quizesCollectionRef, { questions, QuizPostCode: quizCode.substring(10,16), quizTitle: watch().title, owner: sessionStorage.getItem("nick") } );
      setQuestions([])
    }
  ...
  const Logged = () => {
    ...
    const MyForm = () => {
      ...
      <form onSubmit={handleSubmit(questionCreate)} className="quizForm">
        ...
        <input className="quizInput" {...register('title')} placeholder="your title"></input>
